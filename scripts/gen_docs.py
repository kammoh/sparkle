...
# pip3 install -U git+https://github.com/kammoh/symbolator.git
...
from hdlparse.vhdl_parser import VhdlComponent, VhdlEntity
...
def block_diagram(source_files: List[str], options: Options = Options()):
    ...
    all_components = {
        f: [
            (c, vhdl_ex)
            for c in vhdl_ex.extract_objects(f, [VhdlComponent, VhdlEntity])
        ]
        for f in vhdl_files
    }
    ...
    for source, components in all_components.items():
        for comp, extractor in components:
            ...
            print(
                'Creating symbol for {} "{}" -> {}'.format(source, comp.name, fname)
            )
            if options.format == "svg":
                surf = SvgSurface(fname, style, padding=5, scale=options.scale)
            else:
                surf = CairoSurface(fname, style, padding=5, scale=options.scale)

            nc.set_surface(surf)
            nc.clear_shapes()

            sym = make_symbol(
                comp, extractor, options.title, options.libname, options.no_type
            )
            sym.draw(0, 0, nc)
...
block_diagram(["./vhdl/sparkle.vhdl"])
