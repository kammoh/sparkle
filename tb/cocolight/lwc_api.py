...
class LwcCffi():
    ...
    def build_cffi(self, root_cref_dir: Path, algorithms: Dict[str, str], cffi_build_dir: str, DEBUG_LEVEL: int = 0):
        ...
        for op, algorithm in algorithms.items():
            ...
            cref_dir.mkdir(exist_ok=True, parents=True)
            ...
            if DEBUG_LEVEL:
                print("cffi compile with DEBUG=1")
                define_macros.extend([('VERBOSE_LEVEL', DEBUG_LEVEL), ('DEBUG', 1)])
            sources = [str(s) for s in cref_dir.glob("*.c")]
            print(f"cref_dir={cref_dir} sources={sources}")
