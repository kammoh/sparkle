...
class Cref(LwcCffi, LwcAead, LwcHash):
    ...
    def __init__(self, aead_algorithm, hash_algorithm=None, root_cref_dir=Path(SCRIPT_DIR).parent / 'cref') -> None:
...
ref = Cref(aead_algorithm='schwaemm256128v2')
block_bits = dict(AD=256, PT=256)
...
async def debug_enc(dut: HierarchyObject):
    ...
    await tb.xenc_test(ad_size=1,  pt_size=2)
...
async def debug_dec(dut: HierarchyObject):
    ...
    await tb.join_drivers(80000)
    await tb.join_monitors(80000)
