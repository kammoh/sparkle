...
parser.add_argument('--vppreproc', action='store_true',
                    help='Cleanup generated Verilog using vppreproc')
...
def bsc_generate_verilog():
    ...
    print(f"original verilog_sources={verilog_sources}")
    ...
    if args.vppreproc:
        # install perl5. on homebrew: brew install perl
        # make sure homebrew versions of perl & cpan are in your path
        # install Verlog::Perl:  $ cpan install Verilog::Language
        cmd = ['vppreproc', '--noline', '--synthesis', str(out_file)]
        try:
            out = subprocess.run(
                cmd, check=True, stdout=subprocess.PIPE).stdout
            out_file = vout_dir / f'{top}.v'
            with open(out_file, 'wb') as f:
                f.write(out)
        except Exception as e:
    ...
    verilog_sources = [out_file]

    return verilog_sources, top


def test_verilator():
    verilog_sources, top = bsc_generate_verilog()
    if not args.gen:
        run_sim(verilog_sources, top)


def run_sim(verilog_sources, top):
    extra_args = [
